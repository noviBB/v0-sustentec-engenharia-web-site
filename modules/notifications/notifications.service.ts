...
import {
  markPendenciasSeen as markPendenciasSeenRepo,
  markProcessPendenciasSeen as markProcessPendenciasSeenRepo,
...
export async function markProcessPendenciasSeen(
  ...
  try {
    await markProcessPendenciasSeenRepo(session, processId);
    return { kind: ResultKind.Ok };
  } catch (err) {
    const ref = randomUUID().slice(0, 8);
    console.error(
